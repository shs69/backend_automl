...
async def train_classification(file: UploadFile = File(...), target_column: str = ""):
    try:
        result = train_classification_model(file, target_column)
        return result
...
async def predict_classification(file: UploadFile = File(...)):
    try:
        result = predict_classification_model(file)
        return result
...
async def train_regression(file: UploadFile = File(...), target_column: str = ""):
    try:
        result = train_regression_model(file, target_column)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def predict_regression(file: UploadFile = File(...)):
    try:
        result = predict_regression_model(file)
        return result
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
